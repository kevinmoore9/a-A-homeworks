...
def top_titles
  ...
  Movie.select(:title, :id).where("score >= 9.0")
...
def star_wars
  ...
  # hint: use 'select' and 'where'
  Movie.select(:title, :id, :yr).where("title LIKE '%Star Wars%'")
end
...
def pulp_fiction_actors
  ...
  Actor.select(:id, :name).joins(:movies)
...
def uma_movies
  ...
  Movie.select(:id, :title, :yr)
       .joins(:actors).where("actors.name = 'Uma Thurman'")
       .order('id ASC')
